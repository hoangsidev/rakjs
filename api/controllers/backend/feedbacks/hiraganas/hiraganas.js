module.exports = {
  ...
  friendlyName: 'Feedback hiraganas',


  description: 'Feedback hiraganas backend.',
  ...
  exits: {
    success: {
      responseType: 'view',
      viewTemplatePath: 'backend/feedbacks/hiraganas/hiraganas'
    },
    redirect: {
 ...
 var page_title = 'Danh sách Hiragana cần duyệt';
 var page_slug = 'feedbacks_hiragana';
 var per_page = 200;
 var page = inputs.page || 1;
 var data_words = await Words.find({ 'feedback_hiraganas.active' : [ 0, 1 ] }).skip((per_page * page) - per_page).limit(per_page).sort('id DESC').meta({enableExperimentalDeepTargets:true});
//  console.log(data_words)
 var count_words = await  Words.count({ 'feedback_hiraganas.active' : [ 0, 1 ] }).meta({enableExperimentalDeepTargets:true});
