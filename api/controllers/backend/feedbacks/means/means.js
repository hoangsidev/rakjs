module.exports = {
  ...
  exits: {
    success: {
      responseType: 'view',
      viewTemplatePath: 'backend/feedbacks/means/means'
    },
    redirect: {
 ...
 var page_title = 'Danh sách ý nghĩa cần duyệt';
 var page_slug = 'feedbacks_mean';
 var per_page = 200;
 var page = inputs.page || 1;
 var data_words = await Words.find({ 'means.active' : [ 0, 1 ] }).skip((per_page * page) - per_page).limit(per_page).sort('id DESC').meta({enableExperimentalDeepTargets:true});
//  console.log(data_words)
 var count_words = await  Words.count({ 'means.active' : [ 0, 1 ] }).meta({enableExperimentalDeepTargets:true});
