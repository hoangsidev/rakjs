module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var del_feedback_kanji = await Words.update({ id: inputs.id }).set({ feedback_kanjis : [] }).fetch();

    if (data_word) {
      console.log(data_word[0].id); 


      return exits.redirect('/backend/feedbacks_kanji/page/1');
    }
  }
};
